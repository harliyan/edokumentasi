...
class File extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $config['base_url']     = site_url('file/index');                       //arah site dan function nya
    $config['total_rows']   = $this->db->count_all('tb_doc_file');          //total row
    $config['per_page']     = 8;                                            //show data per halaman
    $choice                 = $config["total_rows"] / $config["per_page"];
    $config["num_links"]    = floor($coba);

    if ($keyword == null) {
      $data['doc_file']  = $this->M_file->tampil($config['per_page'], $data['page']);
    } else {
      $data['doc_file']  = $this->M_file->search($keyword);
    }
    ...
    public function tambah_aksi()
  {
    ...
    $this->M_file->input_nik($data, 'tb_record_file');
    redirect('file/download/' . $id);
  ...
  {
    $tb_doc_file = $this->M_file->getDetail('tb_doc_file', $id);

    $data['tb_doc_file'] = $tb_doc_file;
    $data['active_controller'] = 'file';
  ...
  {
    $tb_doc_file = $this->M_file->download('tb_doc_file', $id);
